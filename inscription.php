...
<body>
    ...
    if (!empty($_POST["form_inscription"])) {
        ...
        $select = $db->prepare("SELECT * FROM utilisateur WHERE name_user=:name_user;");
        ...
        if (empty($select->fetch(PDO::FETCH_COLUMN))) {
            ...
            $insert = $db->prepare("INSERT INTO utilisateur(name_user, first_name_user, login_user, email_user, pass_user)
                                    VALUES(:name_user, :first_name_user, :login_user, :email_user, :pass_user)");
