    ...
    <section class="contact">
      <div class="container1" id="inscription">
        <form action="mail3.php" method="post" id="myForm" onsubmit="resetForm()" autocomplete="off">
            ...
            <select class="form-select" aria-label="Default select example" name="formation" required id="formation" onchange="updateModules()">
            ...
            <select class="form-select" aria-label="Default select example" name="modules" id="modules">
              <option value="" disabled selected>Choisir des modules</option>
              <optgroup label="Formation 1 : Former les équipes commerciales aux techniques de vente" id="modulesGroup">
              ...
              <optgroup label="Formation 2 : Gérer mon entreprise au quotidien" id="modulesGroup">
...
function updateModules() {
  ...
  var modulesGroup = document.getElementById("modulesGroup");

  // Récupérer la valeur sélectionnée dans le premier menu déroulant
  var formationValue = formationSelect.value;

  // Effacer les options existantes du deuxième menu déroulant
  modulesSelect.innerHTML = "";

  // Vérifier la valeur sélectionnée et ajouter les options correspondantes
  if (formationValue === "Former les équipes commerciales aux techniques de vente") {
    var modulesOptions = [
      { value: "1", label: "Organisation préalable à la force de vente" },
      { value: "2", label: "La force de vente" },
      { value: "3", label: "Entretien achat/vente" }
    ];
  } else if (formationValue === "Gérer mon entreprise au quotidien") {
    var modulesOptions = [
      { value: "4", label: "Création d'entreprise: aspect juridique" },
      { value: "5", label: "Analyser et interpréter les chiffres comptables" },
      { value: "6", label: "Identifier les solutions à mettre en œuvre et utiliser les leviers d’aide à la décision" },
      { value: "7", label: "Combiner rentabilité, solvabilité et sécurité financière" }
    ];
  }
  ...
  for (var i = 0; i < modulesOptions.length; i++) {
    ...
    option.value = modulesOptions[i].value;
    option.text = modulesOptions[i].label;
    modulesSelect.appendChild(option);
  }
}

// Appeler la fonction une fois au chargement de la page pour initialiser les options
updateModules();
</script>
